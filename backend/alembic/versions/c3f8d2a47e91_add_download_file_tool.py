"""add_download_file_tool

Revision ID: c3f8d2a47e91
...
DOWNLOAD_FILE_TOOL = {
    "name": "DownloadFileTool",
    "display_name": "Download File",
    "description": (
        "The Download File Action lets the agent fetch files (images, PDFs, ...) "
        "from direct URLs and attach them to the chat. It runs with browser-like "
        "request protection so downloads succeed where the sandboxed Python "
        "environment is blocked."
    ),
    "in_code_tool_id": "DownloadFileTool",
    "enabled": True,
}
...
def upgrade() -> None:
    ...
    existing = conn.execute(
        ...
        {"in_code_tool_id": DOWNLOAD_FILE_TOOL["in_code_tool_id"]},
    ).fetchone()
