...
# Download starting, needs to allocate a placeholder block for it on the UI
class DownloadToolStart(BaseObj):
    type: Literal["download_tool_start"] = StreamingType.DOWNLOAD_TOOL_START.value


# A file saved by the download tool, with enough info to render it inline
class DownloadToolFile(BaseModel):
    filename: str
    file_id: str


# Final result of the download tool: saved files + per-URL failure reasons
class DownloadToolFinal(BaseObj):
    type: Literal["download_tool_final"] = StreamingType.DOWNLOAD_TOOL_FINAL.value

    files: list[DownloadToolFile] = []
    # "url (reason)" strings for URLs that could not be downloaded
    failures: list[str] = []


# Image analysis starting, needs to allocate a placeholder block for it on the UI
class AnalyzeImageStart(BaseObj):
    type: Literal["analyze_image_start"] = StreamingType.ANALYZE_IMAGE_START.value


# A file saved and analyzed by the analyze_image tool
class AnalyzeImageFile(BaseModel):
    filename: str
    file_id: str
...
# Final result of the analyze_image tool: analyzed images + per-URL failures
class AnalyzeImageFinal(BaseObj):
    type: Literal["analyze_image_final"] = StreamingType.ANALYZE_IMAGE_FINAL.value

    files: list[AnalyzeImageFile] = []
...
class PythonToolGeneratedFile(BaseModel):
    """A file generated by the Python tool, with enough info to render it."""
...
class PythonToolDelta(BaseObj):
    ...
    stderr: str = ""
    file_ids: list[str] = []
    # Same files as file_ids, with filenames, so the UI can render images
    # inline. Optional so persisted packets from before this field stay valid.
    files: list[PythonToolGeneratedFile] = []
